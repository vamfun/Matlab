%Matlab code swerve angle PID simulation 
...
vx_joy = 1; vy_joy = 0; omega_joy = pi % swerve velocity commands in m/s,omega commands in rad/s

nstep =T/dt+1;
prnt_dt= 1
pose_dt = .75 % how often robot pose is plotted
...
fudge = .05 %used for experimental compensation 
...
%swerve velocity control PID constants 
kff=1
Kp =10 %nominal value 10 
...
%generate wheel speed and angle commands from joy stick. also speed and angle rates for kicks
       [vwc, alphac ,v_wheels_ratec,alpha_wheels_ratec] = ComputeSwerveCommandsfromJoy(vx_joy, vy_joy, omega_joy ,psi, alphac,wheel_positions,vwmax);
%ignore commented out lines
%here i try a few ideas to avoid increasing loop rate by adding some lead to the regular commands       
% modify wheel speeds to compensate for motor lags 
         %fudge= % 1 -.0426*abs(omega_joy)*.69  % empirical derived 
        %vwc = vwc + fudge*v_wheels_ratec*swerve_cmd_loop_dt/2;
          %alphac = alphac + alpha_wheels_ratec*.035;
       %vwc = vwc + fudge*omega_joy*swerve_cmd_loop_dt/2;
   end
      
          
      [vw, alpha] = fieldOrientedToWheelVelocities(Vx, Vy, w, psi , alpha,wheel_positions);
          
             %speed loop PID  calculations ********************
    err=1*(vwc-vw)/vwmax ; 
    errd = err-err_last;   
    err_last=err;
